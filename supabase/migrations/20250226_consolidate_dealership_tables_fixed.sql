...
-- Fixed version: Checks if policies already exist before creating them
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND policyname = 'Users can view their own dealership records'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'dealerships' 
        AND policyname = 'Users can create dealership records'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'dealerships' 
        AND policyname = 'Users can update their own pending dealership records'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'dealerships' 
        AND policyname = 'Admins can view all dealership records'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'dealerships' 
        AND policyname = 'Admins can update any dealership record'
    ) THEN
...
-- 7. Create or replace functions for dealership approval/rejection
-- Function to approve dealership
...
CREATE VIEW dealership_profiles_view AS
SELECT
    id,
    user_id,
    business_name,
